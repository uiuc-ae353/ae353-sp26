...
class Simulator:
    ...
    def pybullet_snapshot(self):
        ...
        o_body_in_world = np.reshape(np.array(pos), (3, 1))
        R_body_in_world = np.reshape(np.array(self.bullet_client.getMatrixFromQuaternion(ori)), (3, 3))
        # PyBullet expects each camera vector to be a flat length-3 sequence.
        p_eye = o_body_in_world.flatten().tolist()
        p_target = (o_body_in_world + R_body_in_world @ np.array([[10.0], [0.], [0.]])).flatten().tolist()
        v_up = (R_body_in_world[:, 2]).flatten().tolist()
        ...
        # spacecraft view
        p_eye = (1.1 * np.array([-3., -4., 4.])).tolist()
        p_target = np.array([0., 0., 0.]).tolist()
        v_up = np.array([0., 0., 1.]).tolist()
        view_matrix = self.bullet_client.computeViewMatrix(p_eye, p_target, v_up)
